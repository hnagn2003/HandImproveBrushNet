...
def get_validation_data(folder_path):
    images_path = os.path.join(folder_path, "images")
    captions_path = os.path.join(folder_path, "caption.jsonl")
    valid_data = []
    ...
    for data in valid_data:
        validation_prompts.append(data['caption'])
        validation_images.append(os.path.join(images_path, data['img_path']))
        validation_masks.append(os.path.join(images_path, data['mask_path']))
...
def log_validation(
    ...
    validation_prompts, validation_images, validation_masks = get_validation_data(args.validation_folder)
    for j, (validation_prompt, validation_image, validation_mask) in enumerate(zip(validation_prompts, validation_images, validation_masks)):
        ...
        for i in range(args.num_validation_images):
            ...
            # Set the caption as the filename and image size
        ...
        plt.tight_layout()
        save_path = os.path.join(args.output_dir, f"valid_checkpoint-{step}-{j}.png")
        # Save the figure
        plt.savefig(save_path)
        print(f"Validation images at step {step} were saved to {save_path}")
        image_logs.append(
...
def parse_args(input_args=None):
    ...
    if args.dataset_name is None and args.train_data_dir is None and args.train_data_type is None:
        raise ValueError("Specify either `--dataset_name` or `--train_data_dir`")

    if args.dataset_name is not None and args.train_data_dir is not None and args.train_data_type is not None:
...
def main(args):
    ...
    train_dataset = get_dataset(
        dataset_names=args.train_data_type,
    ...
    def collate_fn(examples):
        ...
        input_ids = torch.stack([tokenize_captions(example['text'])[0] for example in examples])
    ...
    train_dataloader_len=len(train_dataset)//args.train_batch_size
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    ...
                    if (args.validation_prompt is not None or args.validation_folder) and global_step % args.validation_steps == 0:
    ...
    if accelerator.is_main_process:
        ...
        image_logs = None
        if args.validation_prompt or args.validation_folder is not None:
            image_logs = log_validation(
                vae=vae,
                text_encoder=text_encoder,
                tokenizer=tokenizer,
