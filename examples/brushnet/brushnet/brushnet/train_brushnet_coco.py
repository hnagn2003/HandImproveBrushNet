...
def main(args):
    ...
    def collate_fn(examples):
        ...
        for example in examples:
            ...
            image = example['image']
            mask = example['mask']
            keypoint = example['kpts']
            masked_image=image*mask

            if width>height:
                scale=args.resolution/height          
            else:
                scale=args.resolution/width
            ...
            random_crop=[random.randint(0,w_new-args.resolution),random.randint(0,h_new-args.resolution)]
            import matplotlib.pyplot as plt

            # image=image[random_crop[0]:random_crop[0]+args.resolution,random_crop[1]:random_crop[1]+args.resolution,:]
            # masked_image=masked_image[random_crop[0]:random_crop[0]+args.resolution,random_crop[1]:random_crop[1]+args.resolution,:]
            # mask=mask[random_crop[0]:random_crop[0]+args.resolution,random_crop[1]:random_crop[1]+args.resolution,:]
            # print(random_crop[1], random_crop[0])
            
            for j, _ in enumerate(keypoint):
            # Create a figure and axis
                for i in range(21):
                    keypoint[j][i*3] = keypoint[j][i*3]*scale
                    keypoint[j][i*3+1] = keypoint[j][i*3+1]*scale
                    # keypoint[j][i*3] = keypoint[j][i*3]*w_new/width
                    # keypoint[j][i*3+1] = keypoint[j][i*3+1]*h_new/height
                    keypoint[j][i*3+2] = keypoint[j][i*3+2]+1
                fig, ax = plt.subplots()
                ...
                # Overlay the points
                ax.scatter(keypoint[j][1::3], keypoint[j][0::3], color='red', s=2)  # s is the size of the points

                # Optionally, add labels to the points
                # for i, point in enumerate(keypoint[j]):
                #     ax.text(point[0], point[1], f'Point {i}', color='white', fontsize=12, ha='right')

                
                plt.savefig(str(j) + 'fig.png')
                print("fig is saved")
                exit()
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(brushnet):
                ...
                print(torch.min(conditioning_latents), torch.max(conditioning_latents))
