...
OUTPUT_DIR = '/lustre/scratch/client/vinai/users/ngannh9/hand_improve/BrushNet/output/brushnet_sdxl/'
...
for i in range(len(captions)):
    ...
    mask_image = 1.*(cv2.imread(mask_file).sum(-1)>255)
    # init_image=cv2.resize(init_images,(IMG_SIZE,IMG_SIZE))
    mask_image=cv2.resize(mask_image,(IMG_SIZE,IMG_SIZE))[:,:,np.newaxis]
